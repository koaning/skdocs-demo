...
@app.cell
def _(rng, x, y_true_mean):
    y_normal = y_true_mean + rng.normal(loc=0, scale=0.5 + 0.5 * x, size=x.shape[0])
    a = 5
    y_pareto = y_true_mean + 10 * (rng.pareto(a, size=x.shape[0]) - 1 / (a - 1))
    return y_normal, y_pareto
...
@app.cell
def _(x, y_normal, y_pareto, y_true_mean):
...
@app.cell
def _(X, np, y_normal, y_true_mean):
    from sklearn.linear_model import QuantileRegressor
    _quantiles = [0.05, 0.5, 0.95]
    predictions = {}
    out_bounds_predictions = np.zeros_like(y_true_mean, dtype=np.bool_)
    for _quantile in _quantiles:
        _qr = QuantileRegressor(quantile=_quantile, alpha=0)
        _y_pred = _qr.fit(X, y_normal).predict(X)
        predictions[_quantile] = _y_pred
        if _quantile == min(_quantiles):
            out_bounds_predictions = np.logical_or(out_bounds_predictions, _y_pred >= y_normal)
        elif _quantile == max(_quantiles):
            out_bounds_predictions = np.logical_or(out_bounds_predictions, _y_pred <= y_normal)
    return QuantileRegressor, out_bounds_predictions, predictions
...
@app.cell
def _(X, out_bounds_predictions, plt, predictions, x, y_normal, y_true_mean):
    plt.plot(X, y_true_mean, color='black', linestyle='dashed', label='True mean')
    for _quantile, _y_pred in predictions.items():
        plt.plot(X, _y_pred, label=f'Quantile: {_quantile}')
    plt.scatter(x[out_bounds_predictions], y_normal[out_bounds_predictions], color='black', marker='+', alpha=0.5, label='Outside interval')
    plt.scatter(x[~out_bounds_predictions], y_normal[~out_bounds_predictions], color='black', alpha=0.5, label='Inside interval')
    plt.legend()
    plt.xlabel('x')
    plt.ylabel('y')
...
@app.cell
def _(QuantileRegressor, X, np, y_pareto, y_true_mean):
    _quantiles = [0.05, 0.5, 0.95]
    predictions_1 = {}
    out_bounds_predictions_1 = np.zeros_like(y_true_mean, dtype=np.bool_)
    for _quantile in _quantiles:
        _qr = QuantileRegressor(quantile=_quantile, alpha=0)
        _y_pred = _qr.fit(X, y_pareto).predict(X)
        predictions_1[_quantile] = _y_pred
        if _quantile == min(_quantiles):
            out_bounds_predictions_1 = np.logical_or(out_bounds_predictions_1, _y_pred >= y_pareto)
        elif _quantile == max(_quantiles):
            out_bounds_predictions_1 = np.logical_or(out_bounds_predictions_1, _y_pred <= y_pareto)
    return out_bounds_predictions_1, predictions_1


@app.cell
def _(
    X,
    out_bounds_predictions_1,
    plt,
    predictions_1,
    x,
    y_pareto,
    ...
    plt.plot(X, y_true_mean, color='black', linestyle='dashed', label='True mean')
    for _quantile, _y_pred in predictions_1.items():
        plt.plot(X, _y_pred, label=f'Quantile: {_quantile}')
    plt.scatter(x[out_bounds_predictions_1], y_pareto[out_bounds_predictions_1], color='black', marker='+', alpha=0.5, label='Outside interval')
    plt.scatter(x[~out_bounds_predictions_1], y_pareto[~out_bounds_predictions_1], color='black', alpha=0.5, label='Inside interval')
    plt.legend()
    plt.xlabel('x')
    plt.ylabel('y')
...
@app.cell
def _(QuantileRegressor, X, y_pareto):
    ...
    return linear_regression, quantile_regression
...
@app.cell
def _(X, linear_regression, quantile_regression, y_pareto):
