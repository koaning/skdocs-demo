...
def _():
    ...
    from sklearn.model_selection import HalvingRandomSearchCV  # noqa: F401
    _rng = np.random.RandomState(0)
    _X, _y = make_classification(n_samples=700, random_state=_rng)
    clf = RandomForestClassifier(n_estimators=10, random_state=_rng)
    ...
    rsh = HalvingRandomSearchCV(estimator=clf, param_distributions=param_dist, factor=2, random_state=_rng)
    rsh.fit(_X, _y)
    rsh.best_params_
    return (np,)
...
def _(np):
    ...
    random_unlabeled_points = _rng.rand(iris.target.shape[0]) < 0.3
...
def _():
    ...
    _X, _y = load_iris(return_X_y=True, as_frame=True)
...
def _():
    ...
    _X, _y = fetch_covtype(return_X_y=True)
    pipe = make_pipeline(MinMaxScaler(), PolynomialCountSketch(degree=2, n_components=300), LogisticRegression(max_iter=1000))
    X_train, X_test, y_train, y_test = train_test_split(_X, _y, train_size=5000, test_size=10000, random_state=42)
...
def _():
    ...
    _X, _y = fetch_california_housing(return_X_y=True, as_frame=True)
    features = ['MedInc', 'AveOccup', 'HouseAge', 'AveRooms']
    est = RandomForestRegressor(n_estimators=10)
    est.fit(_X, _y)
    display = PartialDependenceDisplay.from_estimator(est, _X, features, kind='individual', subsample=50, n_jobs=3, grid_resolution=20, random_state=0)
    display.figure_.suptitle('Partial dependence of house value on non-location features\nfor the California housing dataset, with BayesianRidge')
...
def _(np, train_test_split):
    ...
    _rng = np.random.RandomState(0)
    _X = _rng.randn(n_samples, n_features)
    _y = _rng.poisson(lam=np.exp(_X[:, 5]) / 2)
    X_train_1, X_test_1, y_train_1, y_test_1 = train_test_split(_X, _y, random_state=_rng)
