...
@app.cell
def _(ConfusionMatrixDisplay, X_test, X_train, classifier_05, y_test, y_train):
...
@app.cell
def _(X_test, classifier_05, y_test):
...
@app.cell
def _(X, X_test, classifier_05, custom_score, y, y_test):
...
def _():
    ...
    X_1 = make_low_rank_matrix(n_samples=10000, n_features=100, tail_strength=0.1, random_state=0)
    _pca = PCA(n_components=10, svd_solver='covariance_eigh').fit(X_1)
    print(f'Explained variance: {_pca.explained_variance_ratio_.sum():.2f}')
    return (PCA,)
...
def _(PCA):
    ...
    X_2 = random(10000, 100, format='csr', random_state=0)
    _pca = PCA(n_components=10, svd_solver='covariance_eigh').fit(X_2)
    print(f'Explained variance: {_pca.explained_variance_ratio_.sum():.2f}')
    return
...
def _(np):
    ...
    X_4 = np.array([[-1.1, 1.1, 1.1], [3.9, -1.2, np.nan], [np.nan, 1.3, np.nan], [-0.1, -1.4, -1.4], [-4.9, 1.5, -1.5], [np.nan, 1.6, 1.6]])
...
def _():
    ...
    pairwise_distances(X_5, Y, metric=levenshtein_distance)
    return
