...
def _():
    ...
    return X, n_outliers, n_samples, np
...
@app.cell
def _(X, emp_cov, n_outliers, np, plt, robust_cov):
    ...
    ax.legend([mlines.Line2D([], [], color='tab:blue', linestyle='dashed'), mlines.Line2D([], [], color='tab:orange', linestyle='dotted'), inlier_plot, outlier_plot], ['MLE dist', 'MCD dist', 'inliers', 'outliers'], loc='upper right', borderaxespad=0)
...
@app.cell
def _(X, emp_cov, n_outliers, n_samples, np, plt, robust_cov):
    ...
    ax1.set_ylabel('$\\sqrt[3]{\\rm{(Mahal. dist.)}}$', size=16)
    ...
    ax2.set_ylabel('$\\sqrt[3]{\\rm{(Mahal. dist.)}}$', size=16)
