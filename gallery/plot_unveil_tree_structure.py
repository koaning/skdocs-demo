...
def _():
    ...
    return DecisionTreeClassifier, load_iris, np, plt, train_test_split, tree
...
@app.cell
def _(DecisionTreeClassifier, load_iris, train_test_split):
    iris = load_iris()
    X = iris.data
    y = iris.target
...
def _(clf, np):
    ...
    stack = [(0, 0)]
    while len(stack) > 0:  # start with the root node id (0) and its depth (0)
        _node_id, depth = stack.pop()
        node_depth[_node_id] = depth  # `pop` ensures each node is only visited once
        is_split_node = children_left[_node_id] != children_right[_node_id]
        if is_split_node:
            stack.append((children_left[_node_id], depth + 1))
            stack.append((children_right[_node_id], depth + 1))  # If the left and right child of a node is not the same we have a split
        else:  # node
            is_leaves[_node_id] = True
    print('The binary tree structure has {n} nodes and has the following tree structure:\n'.format(n=n_nodes))  # If a split node, append left and right children and depth to `stack`
    for i in range(n_nodes):  # so we can loop through them
        if is_leaves[i]:
            print('{space}node={node} is a leaf node with value={value}.'.format(space=node_depth[i] * '\t', node=i, value=np.around(values[i], 3)))
        else:
            print('{space}node={node} is a split node with value={value}: go to node {left} if X[:, {feature}] <= {threshold} else to node {right}.'.format(space=node_depth[i] * '\t', node=i, left=children_left[i], feature=feature[i], threshold=threshold[i], right=children_right[i], value=np.around(values[i], 3)))
    return feature, n_nodes, threshold
...
@app.cell
def _(X_test, clf, feature, threshold):
    ...
    for _node_id in node_index:
        if leaf_id[sample_id] == _node_id:
            continue
        if X_test[sample_id, feature[_node_id]] <= threshold[_node_id]:
            threshold_sign = '<='
        else:  # continue to the next node if it is a leaf node
            threshold_sign = '>'
        print('decision node {node} : (X_test[{sample}, {feature}] = {value}) {inequality} {threshold})'.format(node=_node_id, sample=sample_id, feature=feature[_node_id], value=X_test[sample_id, feature[_node_id]], inequality=threshold_sign, threshold=threshold[_node_id]))  # check if value of the split feature for sample 0 is below threshold
...
@app.cell
def _(n_nodes, node_indicator, np):
    sample_ids = [0, 1]
