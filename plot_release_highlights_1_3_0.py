...
def _():
    ...
    _X, true_labels = load_digits(return_X_y=True)
    print(f'number of digits: {len(np.unique(true_labels))}')
    hdbscan = HDBSCAN(min_cluster_size=15, copy=True).fit(_X)
...
def _(np):
    ...
    _X = np.array([['cat'] * 30 + ['dog'] * 20 + ['snake'] * 38], dtype=object).T
    _y = [90.3] * 30 + [20.4] * 20 + [21.2] * 38
    _enc = TargetEncoder(random_state=0)
    X_trans = _enc.fit_transform(_X, _y)
    _enc.encodings_
    return
...
def _(np):
    ...
    _y = [0, 0, 1, 1]
    tree = DecisionTreeClassifier(random_state=0).fit(_X, _y)
    tree.predict(_X)
    return
...
def _(np):
    ...
    _X, _y = make_classification(1000, 10, random_state=0)
    _ = ValidationCurveDisplay.from_estimator(LogisticRegression(), _X, _y, param_name='C', param_range=np.geomspace(1e-05, 1000.0, num=9), score_type='both', score_name='Accuracy')
    return
...
def _(np):
    ...
    _X = make_low_rank_matrix(n_samples, n_features, random_state=rng)
    coef = rng.uniform(low=-10, high=20, size=n_features)
    _y = rng.gamma(shape=2, scale=np.exp(_X @ coef) / 2)
    gbdt = HistGradientBoostingRegressor(loss='gamma')
    cross_val_score(gbdt, _X, _y).mean()
    return
...
def _(np):
    ...
    _enc = OrdinalEncoder(min_frequency=6).fit(_X)
    _enc.infrequent_categories_
    return
