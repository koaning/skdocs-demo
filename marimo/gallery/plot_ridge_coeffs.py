...
def _(X, w, y):
    ...
    return alphas, coefs, errors_coefs
...
@app.cell
def _(alphas, coefs, errors_coefs):
    import matplotlib.pyplot as plt
    import pandas as pd
    alphas_1 = pd.Index(alphas, name='alpha')
    coefs_1 = pd.DataFrame(coefs, index=alphas_1, columns=[f'Feature {i}' for i in range(10)])
    errors = pd.Series(errors_coefs, index=alphas_1, name='Mean squared error')
    fig, axs = plt.subplots(1, 2, figsize=(20, 6))
    coefs_1.plot(ax=axs[0], logx=True, title='Ridge coefficients as a function of the regularization strength')
    axs[0].set_ylabel('Ridge coefficient values')
    errors.plot(ax=axs[1], logx=True, title='Coefficient error as a function of the regularization strength')
