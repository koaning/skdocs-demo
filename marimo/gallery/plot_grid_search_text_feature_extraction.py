...
@app.cell
def _(data_train, random_search):
...
@app.cell
def _(parameter_grid, random_search):
...
def _(random_search):
    ...
    return cv_results, shorten_param
...
@app.cell
def _(cv_results, parameter_grid, shorten_param):
    import plotly.express as px
    param_names = [shorten_param(name) for name in parameter_grid.keys()]
    labels = {'mean_score_time': 'CV Score time (s)', 'mean_test_score': 'CV score (accuracy)'}
    _fig = px.scatter(cv_results, x='mean_score_time', y='mean_test_score', error_x='std_score_time', error_y='std_test_score', hover_data=param_names, labels=labels)
    _fig.update_layout(title={'text': 'trade-off between scoring time and mean test score', 'y': 0.95, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top'})
    _fig
    return labels, param_names, px
...
@app.cell
def _(cv_results, labels, param_names, px):
    import math
    ...
    _fig = px.parallel_coordinates(cv_results[column_results].apply(transform_funcs), color='mean_test_score', color_continuous_scale=px.colors.sequential.Viridis_r, labels=labels)
    # Unigrams are mapped to index 1 and bigrams to index 2
    _fig.update_layout(title={'text': 'Parallel coordinates plot of text classifier pipeline', 'y': 0.99, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top'})
    _fig
    return
