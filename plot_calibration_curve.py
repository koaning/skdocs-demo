...
def _():
    ...
    _lr = LogisticRegression(C=1.0)
    gnb = GaussianNB()
    gnb_isotonic = CalibratedClassifierCV(gnb, cv=2, method='isotonic')
    gnb_sigmoid = CalibratedClassifierCV(gnb, cv=2, method='sigmoid')
    clf_list = [(_lr, 'Logistic'), (gnb, 'Naive Bayes'), (gnb_isotonic, 'Naive Bayes + Isotonic'), (gnb_sigmoid, 'Naive Bayes + Sigmoid')]
    return (
...
def _(
    ...
    _ax_calibration_curve = _fig.add_subplot(_gs[:2, :2])
    _calibration_displays = {}
    for _i, (_clf, _name) in enumerate(clf_list):
        _clf.fit(X_train, y_train)
        _display = CalibrationDisplay.from_estimator(_clf, X_test, y_test, n_bins=10, name=_name, ax=_ax_calibration_curve, color=colors(_i))
        _calibration_displays[_name] = _display
    _ax_calibration_curve.grid()
    _ax_calibration_curve.set_title('Calibration plots (Naive Bayes)')
    _grid_positions = [(2, 0), (2, 1), (3, 0), (3, 1)]
    for _i, (_, _name) in enumerate(clf_list):
        _row, _col = _grid_positions[_i]
        _ax = _fig.add_subplot(_gs[_row, _col])
        _ax.hist(_calibration_displays[_name].y_prob, range=(0, 1), bins=10, label=_name, color=colors(_i))
        _ax.set(title=_name, xlabel='Mean predicted probability', ylabel='Count')
    plt.tight_layout()
    # Add histogram
    plt.show()
    return (colors,)
...
@app.cell
def _(X_test, X_train, clf_list, y_test, y_train):
    ...
    for _i, (_clf, _name) in enumerate(clf_list):
        _clf.fit(X_train, y_train)
        _y_prob = _clf.predict_proba(X_test)
        _y_pred = _clf.predict(X_test)
        _scores['Classifier'].append(_name)
        for _metric in [brier_score_loss, log_loss, roc_auc_score]:
            _score_name = _metric.__name__.replace('_', ' ').replace('score', '').capitalize()
            _scores[_score_name].append(_metric(y_test, _y_prob[:, 1]))
        for _metric in [precision_score, recall_score, f1_score]:
            _score_name = _metric.__name__.replace('_', ' ').replace('score', '').capitalize()
            _scores[_score_name].append(_metric(y_test, _y_pred))
        _score_df = pd.DataFrame(_scores).set_index('Classifier')
        _score_df.round(decimals=3)
    _score_df
    return (
        brier_score_loss,
        defaultdict,
...
def _():
    ...
    class NaivelyCalibratedLinearSVC(LinearSVC):
        ...
        def predict_proba(self, X):
            ...
            return proba
    return (NaivelyCalibratedLinearSVC,)


@app.cell
def _(CalibratedClassifierCV, LogisticRegression, NaivelyCalibratedLinearSVC):
    _lr = LogisticRegression(C=1.0)
    svc = NaivelyCalibratedLinearSVC(max_iter=10000)
    svc_isotonic = CalibratedClassifierCV(svc, cv=2, method='isotonic')
    svc_sigmoid = CalibratedClassifierCV(svc, cv=2, method='sigmoid')
    clf_list_1 = [(_lr, 'Logistic'), (svc, 'SVC'), (svc_isotonic, 'SVC + Isotonic'), (svc_sigmoid, 'SVC + Sigmoid')]
    return (clf_list_1,)
...
def _(
    ...
    _ax_calibration_curve = _fig.add_subplot(_gs[:2, :2])
    _calibration_displays = {}
    for _i, (_clf, _name) in enumerate(clf_list_1):
        _clf.fit(X_train, y_train)
        _display = CalibrationDisplay.from_estimator(_clf, X_test, y_test, n_bins=10, name=_name, ax=_ax_calibration_curve, color=colors(_i))
        _calibration_displays[_name] = _display
    _ax_calibration_curve.grid()
    _ax_calibration_curve.set_title('Calibration plots (SVC)')
    _grid_positions = [(2, 0), (2, 1), (3, 0), (3, 1)]
    for _i, (_, _name) in enumerate(clf_list_1):
        _row, _col = _grid_positions[_i]
        _ax = _fig.add_subplot(_gs[_row, _col])
        _ax.hist(_calibration_displays[_name].y_prob, range=(0, 1), bins=10, label=_name, color=colors(_i))
        _ax.set(title=_name, xlabel='Mean predicted probability', ylabel='Count')
