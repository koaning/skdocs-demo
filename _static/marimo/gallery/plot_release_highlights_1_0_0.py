...
def _():
    ...
    X_1 = pd.DataFrame([[1, 2, 3], [4, 5, 6]], columns=['a', 'b', 'c'])
    scalar = StandardScaler().fit(X_1)
    scalar.feature_names_in_
    return StandardScaler, pd
...
def _(StandardScaler, pd):
    ...
    X_2 = pd.DataFrame({'pet': ['dog', 'cat', 'fish'], 'age': [3, 7, 1]})
    preprocessor = ColumnTransformer([('numerical', StandardScaler(), ['age']), ('categorical', OneHotEncoder(), ['pet'])], verbose_feature_names_out=False).fit(X_2)
    preprocessor.get_feature_names_out()
    return X_2, preprocessor
