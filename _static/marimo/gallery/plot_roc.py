...
def _():
    ...
    return X_test, X_train, n_classes, np, target_names, y, y_test, y_train
...
@app.cell
def _(label_binarizer, np):
    class_of_interest = "virginica"
    class_id = np.flatnonzero(label_binarizer.classes_ == class_of_interest)[0]
    class_id
    return class_id, class_of_interest


@app.cell
def _(class_id, class_of_interest, y_onehot_test, y_score):
    import matplotlib.pyplot as plt
    from sklearn.metrics import RocCurveDisplay
    _display = RocCurveDisplay.from_predictions(y_onehot_test[:, class_id], y_score[:, class_id], name=f'{class_of_interest} vs the rest', curve_kwargs=dict(color='darkorange'), plot_chance_level=True, despine=True)
    _ = _display.ax_.set(xlabel='False Positive Rate', ylabel='True Positive Rate', title='One-vs-Rest ROC curves:\nVirginica vs (Setosa & Versicolor)')
    return RocCurveDisplay, plt
...
@app.cell
def _(RocCurveDisplay, y_onehot_test, y_score):
    _display = RocCurveDisplay.from_predictions(y_onehot_test.ravel(), y_score.ravel(), name='micro-average OvR', curve_kwargs=dict(color='darkorange'), plot_chance_level=True, despine=True)
    _ = _display.ax_.set(xlabel='False Positive Rate', ylabel='True Positive Rate', title='Micro-averaged One-vs-Rest\nReceiver Operating Characteristic')
    return
...
def _(y_onehot_test, y_score):
    ...
    print(f"Micro-averaged One-vs-Rest ROC AUC score:\n{roc_auc['micro']:.2f}")
    return auc, fpr, roc_auc, roc_curve, tpr
...
def _(
    ...
    # Interpolate all ROC curves at these points
        mean_tpr = mean_tpr + np.interp(fpr_grid, fpr[i], tpr[i])
    mean_tpr = mean_tpr / n_classes
    fpr['macro'] = fpr_grid
    tpr['macro'] = mean_tpr  # linear interpolation
...
def _(
    ...
    for class_id_1, color in zip(range(n_classes), colors):
        RocCurveDisplay.from_predictions(y_onehot_test[:, class_id_1], y_score[:, class_id_1], name=f'ROC curve for {target_names[class_id_1]}', curve_kwargs=dict(color=color), ax=_ax, plot_chance_level=class_id_1 == 2, despine=True)
    _ = _ax.set(xlabel='False Positive Rate', ylabel='True Positive Rate', title='Extension of Receiver Operating Characteristic\nto One-vs-Rest multiclass')
    return
...
def _(
    ...
    pair_scores = []
    mean_tpr_1 = dict()
    for _ix, (_label_a, _label_b) in enumerate(pair_list):
        a_mask = y_test == _label_a
        b_mask = y_test == _label_b
        ab_mask = np.logical_or(a_mask, b_mask)
        a_true = a_mask[ab_mask]
        b_true = b_mask[ab_mask]
        idx_a = np.flatnonzero(label_binarizer.classes_ == _label_a)[0]
        idx_b = np.flatnonzero(label_binarizer.classes_ == _label_b)[0]
        fpr_a, tpr_a, _ = roc_curve(a_true, y_score[ab_mask, idx_a])
        fpr_b, tpr_b, _ = roc_curve(b_true, y_score[ab_mask, idx_b])
        mean_tpr_1[_ix] = np.zeros_like(fpr_grid)
        mean_tpr_1[_ix] = mean_tpr_1[_ix] + np.interp(fpr_grid, fpr_a, tpr_a)
        mean_tpr_1[_ix] = mean_tpr_1[_ix] + np.interp(fpr_grid, fpr_b, tpr_b)
        mean_tpr_1[_ix] = mean_tpr_1[_ix] / 2
        mean_score = auc(fpr_grid, mean_tpr_1[_ix])
        pair_scores.append(mean_score)
        _fig, _ax = plt.subplots(figsize=(6, 6))
        plt.plot(fpr_grid, mean_tpr_1[_ix], label=f'Mean {_label_a} vs {_label_b} (AUC = {mean_score:.2f})', linestyle=':', linewidth=4)
        RocCurveDisplay.from_predictions(a_true, y_score[ab_mask, idx_a], ax=_ax, name=f'{_label_a} as positive class')
        RocCurveDisplay.from_predictions(b_true, y_score[ab_mask, idx_b], ax=_ax, name=f'{_label_b} as positive class', plot_chance_level=True, despine=True)
        _ax.set(xlabel='False Positive Rate', ylabel='True Positive Rate', title=f'{target_names[idx_a]} vs {_label_b} ROC curves')
    print(f'Macro-averaged One-vs-One ROC AUC score:\n{np.average(pair_scores):.2f}')
    return mean_tpr_1, pair_scores
...
def _(
    ...
    _fig, _ax = plt.subplots(figsize=(6, 6))
    for _ix, (_label_a, _label_b) in enumerate(pair_list):
        ovo_tpr = ovo_tpr + mean_tpr_1[_ix]
        _ax.plot(fpr_grid, mean_tpr_1[_ix], label=f'Mean {_label_a} vs {_label_b} (AUC = {pair_scores[_ix]:.2f})')
    ovo_tpr = ovo_tpr / sum((1 for pair in enumerate(pair_list)))
    _ax.plot(fpr_grid, ovo_tpr, label=f'One-vs-One macro-average (AUC = {macro_roc_auc_ovo:.2f})', linestyle=':', linewidth=4)
    _ax.plot([0, 1], [0, 1], 'k--', label='Chance level (AUC = 0.5)')
