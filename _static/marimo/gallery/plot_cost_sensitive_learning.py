...
def _():
    ...
    return X, fetch_openml, sklearn, y
...
def _():
    ...
    def fpr_score(y, y_pred, neg_label, pos_label):
        ...
        tnr = tn / (tn + fp)
        return 1 - tnr
    return confusion_matrix, fpr_score
...
@app.cell
def _(fpr_score, neg_label, pos_label):
...
@app.cell
def _(confusion_matrix, make_scorer, neg_label, pos_label, scoring):
    import numpy as np
...
@app.cell
def _(X_test, model, pos_label, scoring, y_test):
    ...
    return PrecisionRecallDisplay, RocCurveDisplay, plt
...
@app.cell
def _(X_test, model, scoring, y_test):
...
@app.cell
def _(X_train, model, scoring, y_train):
    ...
    return TunedThresholdClassifierCV, tuned_model
...
def _(
    ...
    def plot_roc_pr_curves(vanilla_model, tuned_model, *, title):
        ...
        for idx, (est, linestyle, marker, color, name) in enumerate(zip((vanilla_model, tuned_model), linestyles, markerstyles, colors, names)):
            decision_threshold = getattr(est, 'best_threshold_', 0.5)
            PrecisionRecallDisplay.from_estimator(est, X_test, y_test, pos_label=pos_label, linestyle=linestyle, color=color, ax=axs[0], name=name)
            axs[0].plot(scoring['recall'](est, X_test, y_test), scoring['precision'](est, X_test, y_test), marker, markersize=10, color=color, label=f'Cut-off point at probability of {decision_threshold:.2f}')
            RocCurveDisplay.from_estimator(est, X_test, y_test, pos_label=pos_label, curve_kwargs=dict(linestyle=linestyle, color=color), ax=axs[1], name=name, plot_chance_level=idx == 1)
            axs[1].plot(scoring['fpr'](est, X_test, y_test), scoring['tpr'](est, X_test, y_test), marker, markersize=10, color=color, label=f'Cut-off point at probability of {decision_threshold:.2f}')
        axs[0].set_title('Precision-Recall curve')
        axs[0].legend()
        axs[1].set_title('ROC curve')
        axs[1].legend()
        ...
        fig.suptitle(_title)
    return (plot_roc_pr_curves,)


@app.cell
def _(model, plot_roc_pr_curves, tuned_model):
    _title = 'Comparison of the cut-off point for the vanilla and tuned GBDT model'
    plot_roc_pr_curves(model, tuned_model, title=_title)
    return
...
@app.cell
def _(X_test, scoring, tuned_model, y_test):
...
@app.cell
def _(X_train, model, tuned_model, y_train):
...
@app.cell
def _(model, plot_roc_pr_curves, tuned_model):
    _title = 'Tuned GBDT model without refitting and using the entire dataset'
    plot_roc_pr_curves(model, tuned_model, title=_title)
    return
...
@app.cell
def _(X_train, tuned_model, y_train):
...
@app.cell
def _(amount, data, target, train_test_split):
    data_train, data_test, target_train, target_test, amount_train, amount_test = train_test_split(data, target, amount, stratify=target, test_size=0.5, random_state=42)
    return (
        amount_test,
        amount_train,
        data_test,
...
def _(
    ...
    _benefit = business_scorer(always_accept_policy, data_test, target_test, amount=amount_test)
    print(f"Benefit of the 'always accept' policy: {_benefit:,.2f}€")
...
def _(
    ...
    _benefit = business_scorer(always_reject_policy, data_test, target_test, amount=amount_test)
    print(f"Benefit of the 'always reject' policy: {_benefit:,.2f}€")
    return
...
@app.cell
def _(data_train, np, target_train):
    ...
    model_1 = GridSearchCV(logistic_regression, param_grid, scoring='neg_log_loss').fit(data_train, target_train)
    model_1
    return (model_1,)


@app.cell
def _(amount_test, business_scorer, data_test, model_1, target_test):
    print(f'Benefit of logistic regression with default threshold: {business_scorer(model_1, data_test, target_test, amount=amount_test):,.2f}€')
    return
...
@app.cell
def _(TunedThresholdClassifierCV, business_scorer, model_1):
    tuned_model_1 = TunedThresholdClassifierCV(estimator=model_1.best_estimator_, scoring=business_scorer, thresholds=100, n_jobs=2)
    return (tuned_model_1,)
...
@app.cell
def _(amount_train, data_train, target_train, tuned_model_1):
    tuned_model_1.fit(data_train, target_train, amount=amount_train)
    return
...
@app.cell
def _(tuned_model_1):
    print(f'Tuned decision threshold: {tuned_model_1.best_threshold_:.2f}')
    return


@app.cell
def _(amount_test, business_scorer, data_test, target_test, tuned_model_1):
    print(f'Benefit of logistic regression with a tuned threshold: {business_scorer(tuned_model_1, data_test, target_test, amount=amount_test):,.2f}€')
    return
...
@app.cell
def _(model_1, tuned_model_1):
    from sklearn.frozen import FrozenEstimator
    from sklearn.model_selection import FixedThresholdClassifier
    model_fixed_threshold = FixedThresholdClassifier(estimator=FrozenEstimator(model_1), threshold=tuned_model_1.best_threshold_)
    return (model_fixed_threshold,)
