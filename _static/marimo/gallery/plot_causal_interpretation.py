...
def _(X_test, X_train, y_test, y_train):
    ...
    return LinearRegression, features_names, r2_score, regressor_with_ability
...
@app.cell
def _(features_names, pd, regressor_with_ability, true_coef):
    import matplotlib.pyplot as plt
    _model_coef = pd.Series(regressor_with_ability.coef_, index=features_names)
    _coef = pd.concat([true_coef[features_names], _model_coef], keys=['Coefficients of true generative model', 'Model coefficients'], axis=1)
    _ax = _coef.plot.barh()
    _ax.set_xlabel('Coefficient values')
    _ax.set_title('Coefficients of the linear regression including the ability features')
    _ = plt.tight_layout()
    return (plt,)
...
def _(LinearRegression, X_test, X_train, r2_score, y_test, y_train):
    features_names_1 = ['experience', 'parent hourly wage', 'college degree']
    regressor_without_ability = LinearRegression()
    regressor_without_ability.fit(X_train[features_names_1], y_train)
    y_pred_without_ability = regressor_without_ability.predict(X_test[features_names_1])
    R2_without_ability = r2_score(y_test, y_pred_without_ability)
    print(f'R2 score without ability: {R2_without_ability:.3f}')
    return features_names_1, regressor_without_ability
...
@app.cell
def _(features_names_1, pd, plt, regressor_without_ability, true_coef):
    _model_coef = pd.Series(regressor_without_ability.coef_, index=features_names_1)
    _coef = pd.concat([true_coef[features_names_1], _model_coef], keys=['Coefficients of true generative model', 'Model coefficients'], axis=1)
    _ax = _coef.plot.barh()
    _ax.set_xlabel('Coefficient values')
    _ = _ax.set_title('Coefficients of the linear regression excluding the ability feature')
    plt.tight_layout()
    plt.show()
    return
