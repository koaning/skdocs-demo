...
def _(np):
    ...
    from sklearn.impute import IterativeImputer, KNNImputer, SimpleImputer  # noqa: F401
    ...
    def get_score(X, y, imputer=None):
        ...
        if _imputer is not None:
            estimator = make_pipeline(_imputer, regressor)
        else:
            estimator = regressor
...
def _(
    ...
    _imputer = SimpleImputer(strategy='constant', fill_value=0, add_indicator=True)
    mses_diabetes[1], stds_diabetes[1] = get_score(X_miss_diabetes, y_miss_diabetes, _imputer)
    mses_california[1], stds_california[1] = get_score(X_miss_california, y_miss_california, _imputer)
...
def _(
    ...
    _imputer = SimpleImputer(strategy='mean', add_indicator=True)
    mses_diabetes[2], stds_diabetes[2] = get_score(X_miss_diabetes, y_miss_diabetes, _imputer)
    mses_california[2], stds_california[2] = get_score(X_miss_california, y_miss_california, _imputer)
...
def _(
    ...
    _imputer = KNNImputer(add_indicator=True)
    mses_diabetes[3], stds_diabetes[3] = get_score(X_miss_diabetes, y_miss_diabetes, _imputer)
    mses_california[3], stds_california[3] = get_score(X_miss_california, y_miss_california, make_pipeline(RobustScaler(), _imputer))
...
def _(
    ...
    _imputer = IterativeImputer(add_indicator=True)
    mses_diabetes[4], stds_diabetes[4] = get_score(X_miss_diabetes, y_miss_diabetes, _imputer)
    mses_california[4], stds_california[4] = get_score(X_miss_california, y_miss_california, make_pipeline(RobustScaler(), _imputer))
    x_labels.append('Iterative Imputation')
    mses_diabetes_1 = mses_diabetes * -1
    mses_california_1 = mses_california * -1
    return mses_california_1, mses_diabetes_1
...
def _(
    ...
    # plot diabetes results
        ax1.barh(j, mses_diabetes_1[j], xerr=stds_diabetes[j], color=colors[j], alpha=0.6, align='center')
    ax1.set_title('Imputation Techniques with Diabetes Data')
    ax1.set_xlim(left=np.min(mses_diabetes_1) * 0.9, right=np.max(mses_diabetes_1) * 1.1)
    ax1.set_yticks(xval)
    ax1.set_xlabel('MSE')
    ax1.invert_yaxis()
    ...
    ax2 = plt.subplot(122)
    for j in xval:
        ax2.barh(j, mses_california_1[j], xerr=stds_california[j], color=colors[j], alpha=0.6, align='center')
