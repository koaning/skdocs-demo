...
def _():
    ...
    return categories, fetch_20newsgroups, load_dataset, time
...
def _(X_test, X_train, y_train):
    ...
    return RidgeClassifier, clf, pred
...
@app.cell
def _(clf, pred, target_names, y_test):
    import matplotlib.pyplot as plt
    from sklearn.metrics import ConfusionMatrixDisplay
    _fig, _ax = plt.subplots(figsize=(10, 5))
    ConfusionMatrixDisplay.from_predictions(y_test, pred, ax=_ax)
    _ax.xaxis.set_ticklabels(target_names)
    _ax.yaxis.set_ticklabels(target_names)
    _ = _ax.set_title(f'Confusion Matrix for {clf.__class__.__name__}\non the original documents')
    return ConfusionMatrixDisplay, plt
...
@app.cell
def _(X_train, clf, feature_names, plt, target_names):
    import numpy as np
    import pandas as pd

    def plot_feature_effects():
        average_feature_effects = clf.coef_ * np.asarray(X_train.mean(axis=0)).ravel()
        for i, label in enumerate(target_names):  # learned coefficients weighted by frequency of appearance
        ...
        for i, label in enumerate(target_names):  # plot feature effects
            _ax.barh(y_locs + (i - 2) * bar_size, average_feature_effects[i, top_indices], height=bar_size, label=label)
        _ax.set(yticks=y_locs, yticklabels=predictive_words, ylim=[0 - 4 * bar_size, len(top_indices) * (4 * bar_size + padding) - 4 * bar_size])
        _ax.legend(loc='lower right')
        print('top 5 keywords per class:')
        print(top)
        return _ax
    _ = plot_feature_effects().set_title('Average feature effect on the original data')
    return np, plot_feature_effects
...
@app.cell
def _(categories, fetch_20newsgroups):
    data_train = fetch_20newsgroups(
...
@app.cell
def _(ConfusionMatrixDisplay, RidgeClassifier, load_dataset, plt):
    X_train_1, X_test_1, y_train_1, y_test_1, feature_names_1, target_names_1 = load_dataset(remove=('headers', 'footers', 'quotes'))
    clf_1 = RidgeClassifier(tol=0.01, solver='sparse_cg')
    clf_1.fit(X_train_1, y_train_1)
    pred_1 = clf_1.predict(X_test_1)
    _fig, _ax = plt.subplots(figsize=(10, 5))
    ConfusionMatrixDisplay.from_predictions(y_test_1, pred_1, ax=_ax)
    _ax.xaxis.set_ticklabels(target_names_1)
    _ax.yaxis.set_ticklabels(target_names_1)
    _ = _ax.set_title(f'Confusion Matrix for {clf_1.__class__.__name__}\non filtered documents')
...
@app.cell
def _(X_test_1, X_train_1, time, y_test_1, y_train_1):
...
@app.cell
def _(RidgeClassifier, benchmark):
    ...
    results = []
    for clf_2, name in ((LogisticRegression(C=5, max_iter=1000), 'Logistic Regression'), (RidgeClassifier(alpha=1.0, solver='sparse_cg'), 'Ridge Classifier'), (KNeighborsClassifier(n_neighbors=100), 'kNN'), (RandomForestClassifier(), 'Random Forest'), (LinearSVC(C=0.1, dual=False, max_iter=1000), 'Linear SVC'), (SGDClassifier(loss='log_loss', alpha=0.0001, n_iter_no_change=3, early_stopping=True), 'log-loss SGD'), (NearestCentroid(), 'NearestCentroid'), (ComplementNB(alpha=0.1), 'Complement naive Bayes')):
        print('=' * 80)
        print(name)
        results.append(benchmark(clf_2, name))  # L2 penalty Linear SVC  # L2 penalty Linear SGD  # NearestCentroid (aka Rocchio classifier)  # Sparse naive Bayes classifier
    return (results,)
...
def _(np, plt, results):
    ...
    results_1 = [[x[i] for x in results] for i in range(4)]
    clf_names, score, training_time, test_time = results_1
    ...
    _fig, ax2 = plt.subplots(figsize=(10, 8))
