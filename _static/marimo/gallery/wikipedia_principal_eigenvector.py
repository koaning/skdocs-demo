    ...
    for url, filename in resources:
        if not os.path.exists(filename):
            ...
            with open(filename, "wb") as f:
                f.write(opener.read())
            print()
    return page_links_filename, redirects_filename
...
def _(BZ2File, datetime):
    ...
    def get_redirects(redirects_filename):
        ...
        return redirects
    return get_redirects, index, short_name
...
@app.cell
def _(X, names, np, pprint, randomized_svd, time):
    print('Computing the principal singular vectors using randomized_svd')
    _t0 = time()
    U, s, V = randomized_svd(X, 5, n_iter=3)
    print('done in %0.3fs' % (time() - _t0))
...
def _(X, names, np, pprint, time):
    def centrality_scores(X, alpha=0.85, max_iter=100, tol=1e-10):
        ...
        for i in range(max_iter):
            print('power iteration #%d' % i)  # initial guess
            prev_scores = scores
            ...
            if err < n * tol:  # check convergence: normalized l_inf norm
                return scores
        return scores
