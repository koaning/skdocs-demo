...
def _():
    ...
    def plot_obs_pred(df, feature, weight, observed, predicted, y_label=None, title=None, ax=None, fill_legend=False):
        """Plot observed and predicted - aggregated per feature level.  # freMTPL2sev dataset from https://www.openml.org/d/41215
        ...
        weight : str
            column name of df with the values of weights or exposure  # unquote string fields
        observed : str
        ...
        _ax = df_.loc[:, ['observed', 'predicted']].plot(style='.', ax=_ax)
        y_max = df_.loc[:, ['observed', 'predicted']].values.max() * 0.8
        p2 = _ax.fill_between(df_.index, 0, y_max * df_[weight] / df_[weight].values.max(), color='g', alpha=0.1)
        if fill_legend:
            _ax.legend([p2], ['{} distribution'.format(feature)])
        _ax.set(ylabel=y_label if y_label is not None else None, title=title if title is not None else 'Train: Observed vs Predicted')
    ...
    def score_estimator(estimator, X_train, X_test, df_train, df_test, target, weights, tweedie_powers=None):
        ...
        if tweedie_powers:
            metrics = metrics + [('mean Tweedie dev p={:.4f}'.format(power), partial(mean_tweedie_deviance, power=power)) for power in tweedie_powers]
        res = []
        ...
        return res
    return load_mtpl2, np, pd, plot_obs_pred, plt, score_estimator
...
def _(X, df):
    ...
    return PoissonRegressor, X_test, X_train, df_test, df_train
...
@app.cell
def _(PoissonRegressor, X_test, X_train, df_test, df_train, score_estimator):
    glm_freq = PoissonRegressor(alpha=0.0001, solver='newton-cholesky')
    glm_freq.fit(X_train, df_train['Frequency'], sample_weight=df_train['Exposure'])
    _scores = score_estimator(glm_freq, X_train, X_test, df_train, df_test, target='Frequency', weights='Exposure')
...
@app.cell
def _(X_test, X_train, df_test, df_train, glm_freq, plot_obs_pred, plt):
    _fig, _ax = plt.subplots(ncols=2, nrows=2, figsize=(16, 8))
    _fig.subplots_adjust(hspace=0.3, wspace=0.2)
    plot_obs_pred(df=df_train, feature='DrivAge', weight='Exposure', observed='Frequency', predicted=glm_freq.predict(X_train), y_label='Claim Frequency', title='train data', ax=_ax[0, 0])
    plot_obs_pred(df=df_test, feature='DrivAge', weight='Exposure', observed='Frequency', predicted=glm_freq.predict(X_test), y_label='Claim Frequency', title='test data', ax=_ax[0, 1], fill_legend=True)
    plot_obs_pred(df=df_test, feature='VehAge', weight='Exposure', observed='Frequency', predicted=glm_freq.predict(X_test), y_label='Claim Frequency', title='test data', ax=_ax[1, 0], fill_legend=True)
    plot_obs_pred(df=df_test, feature='BonusMalus', weight='Exposure', observed='Frequency', predicted=glm_freq.predict(X_test), y_label='Claim Frequency', title='test data', ax=_ax[1, 1], fill_legend=True)
    return
...
@app.cell
def _(X_test, X_train, df_test, df_train, score_estimator):
    ...
    _scores = score_estimator(glm_sev, X_train[mask_train.values], X_test[mask_test.values], df_train[mask_train], df_test[mask_test], target='AvgClaimAmount', weights='ClaimNb')
    print('Evaluation of GammaRegressor on target AvgClaimAmount')
    print(_scores)
    return glm_sev, mask_test, mask_train
...
def _(
    ...
    _scores = score_estimator(dummy_sev, X_train[mask_train.values], X_test[mask_test.values], df_train[mask_train], df_test[mask_test], target='AvgClaimAmount', weights='ClaimNb')
...
@app.cell
def _(X_train, df_train, dummy_sev, glm_sev):
    print(
...
def _(
    ...
    _fig, _ax = plt.subplots(ncols=1, nrows=2, figsize=(16, 6))
    plot_obs_pred(df=df_train.loc[mask_train], feature='DrivAge', weight='Exposure', observed='AvgClaimAmount', predicted=glm_sev.predict(X_train[mask_train.values]), y_label='Average Claim Severity', title='train data', ax=_ax[0])
    plot_obs_pred(df=df_test.loc[mask_test], feature='DrivAge', weight='Exposure', observed='AvgClaimAmount', predicted=glm_sev.predict(X_test[mask_test.values]), y_label='Average Claim Severity', title='test data', ax=_ax[1], fill_legend=True)
    plt.tight_layout()
    return
...
def _(
    ...
    _scores = pd.concat([scores_product_model, scores_glm_pure_premium], axis=1, sort=True, keys=('Product Model', 'TweedieRegressor'))
    print('Evaluation of the Product Model and the Tweedie Regressor on target PurePremium')
    with pd.option_context('display.expand_frame_repr', False):
        print(_scores)
    return (glm_pure_premium,)
...
def _(
    ...
    res = []
    for subset_label, X_1, df_1 in [('train', X_train, df_train), ('test', X_test, df_test)]:
        exposure = df_1['Exposure'].values
        res.append({'subset': subset_label, 'observed': df_1['ClaimAmount'].values.sum(), 'predicted, frequency*severity model': np.sum(exposure * glm_freq.predict(X_1) * glm_sev.predict(X_1)), 'predicted, tweedie, power=%.2f' % glm_pure_premium.power: np.sum(exposure * glm_pure_premium.predict(X_1))})
...
@app.cell
def _(X_test, df_test, glm_freq, glm_pure_premium, glm_sev, np, plt):
    ...
    def lorenz_curve(y_true, y_pred, exposure):
        ...
        cumulative_claim_amount = cumulative_claim_amount / cumulative_claim_amount[-1]
        cumulative_exposure = np.cumsum(ranked_exposure)
        cumulative_exposure = cumulative_exposure / cumulative_exposure[-1]
    ...
    for label, y_pred in [('Frequency * Severity model', y_pred_product), ('Compound Poisson Gamma', y_pred_total)]:
        ...
        label = label + ' (Gini index: {:.3f})'.format(gini)
        _ax.plot(cum_exposure, cum_claims, linestyle='-', label=label)
    cum_exposure, cum_claims = lorenz_curve(df_test['PurePremium'], df_test['PurePremium'], df_test['Exposure'])
    gini = 1 - 2 * auc(cum_exposure, cum_claims)
    label = 'Oracle (Gini index: {:.3f})'.format(gini)
    _ax.plot(cum_exposure, cum_claims, linestyle='-.', color='gray', label=label)
    _ax.plot([0, 1], [0, 1], linestyle='--', color='black', label='Random baseline')
    _ax.set(title='Lorenz Curves', xlabel='Cumulative proportion of exposure\n(ordered by model from safest to riskiest)', ylabel='Cumulative proportion of claim amounts')
