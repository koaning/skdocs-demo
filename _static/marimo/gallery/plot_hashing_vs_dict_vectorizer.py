...
def _(tokenize):
    ...
    return defaultdict, token_freqs
...
@app.cell
def _(data_size_mb, defaultdict, raw_data, token_freqs):
    ...
    dict_count_vectorizers['speed'].append(data_size_mb / _duration)
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
    print(f'Found {len(vectorizer.get_feature_names_out())} unique terms')
    return dict_count_vectorizers, time, vectorizer
...
def _():
    ...
    def n_nonzero_columns(X):
        ...
        """
        return len(np.unique(_X.nonzero()[1]))
    return n_nonzero_columns, np
...
def _(
    ...
    _t0 = time()
    _hasher = FeatureHasher(n_features=2 ** 18)
    _X = _hasher.transform((token_freqs(d) for d in raw_data))
    _duration = time() - _t0
    dict_count_vectorizers['vectorizer'].append(_hasher.__class__.__name__ + '\non freq dicts')
    dict_count_vectorizers['speed'].append(data_size_mb / _duration)
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
    print(f'Found {n_nonzero_columns(_X)} unique tokens')
...
def _(
    ...
    _t0 = time()
    _hasher = FeatureHasher(n_features=2 ** 22)
    _X = _hasher.transform((token_freqs(d) for d in raw_data))
    _duration = time() - _t0
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
    print(f'Found {n_nonzero_columns(_X)} unique tokens')
...
def _(
    ...
    _t0 = time()
    _hasher = FeatureHasher(n_features=2 ** 18, input_type='string')
    _X = _hasher.transform((tokenize(d) for d in raw_data))
    _duration = time() - _t0
    dict_count_vectorizers['vectorizer'].append(_hasher.__class__.__name__ + '\non raw tokens')
    dict_count_vectorizers['speed'].append(data_size_mb / _duration)
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
...
@app.cell
def _(dict_count_vectorizers, np):
    import matplotlib.pyplot as plt
    _fig, _ax = plt.subplots(figsize=(12, 6))
    _y_pos = np.arange(len(dict_count_vectorizers['vectorizer']))
    _ax.barh(_y_pos, dict_count_vectorizers['speed'], align='center')
    _ax.set_yticks(_y_pos)
    _ax.set_yticklabels(dict_count_vectorizers['vectorizer'])
    _ax.invert_yaxis()
    _ = _ax.set_xlabel('speed (MB/s)')
    return (plt,)
...
@app.cell
def _(data_size_mb, dict_count_vectorizers, raw_data, time):
    from sklearn.feature_extraction.text import CountVectorizer
    _t0 = time()
    vectorizer_1 = CountVectorizer()
    vectorizer_1.fit_transform(raw_data)
    _duration = time() - _t0
    dict_count_vectorizers['vectorizer'].append(vectorizer_1.__class__.__name__)
    dict_count_vectorizers['speed'].append(data_size_mb / _duration)
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
    print(f'Found {len(vectorizer_1.get_feature_names_out())} unique terms')
    return
...
def _(data_size_mb, dict_count_vectorizers, raw_data, time):
    ...
    _t0 = time()
    vectorizer_2 = HashingVectorizer(n_features=2 ** 18)
    vectorizer_2.fit_transform(raw_data)
    _duration = time() - _t0
    dict_count_vectorizers['vectorizer'].append(vectorizer_2.__class__.__name__)
    dict_count_vectorizers['speed'].append(data_size_mb / _duration)
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
    return
...
def _(data_size_mb, dict_count_vectorizers, raw_data, time):
    ...
    vectorizer_3.fit_transform(raw_data)
    _duration = time() - _t0
    dict_count_vectorizers['vectorizer'].append(vectorizer_3.__class__.__name__)
    dict_count_vectorizers['speed'].append(data_size_mb / _duration)
    print(f'done in {_duration:.3f} s at {data_size_mb / _duration:.1f} MB/s')
    print(f'Found {len(vectorizer_3.get_feature_names_out())} unique terms')
    return
...
@app.cell
def _(dict_count_vectorizers, np, plt):
    _fig, _ax = plt.subplots(figsize=(12, 6))
    _y_pos = np.arange(len(dict_count_vectorizers['vectorizer']))
    _ax.barh(_y_pos, dict_count_vectorizers['speed'], align='center')
    _ax.set_yticks(_y_pos)
