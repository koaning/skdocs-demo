...
def _():
    ...
    _X, _y = make_classification(n_samples=1000, random_state=0)
    start = time.time()
    classifier = SGDClassifier().fit(_X, _y)
    print(f'Fitting the classifier took {(time.time() - start) * 1000:.2f} milliseconds')
    start = time.time()
    threshold_classifier = FixedThresholdClassifier(estimator=FrozenEstimator(classifier), threshold=0.9).fit(_X, _y)
    print(f'Fitting the threshold classifier took {(time.time() - start) * 1000:.2f} milliseconds')
    return
...
def _():
    ...
    _y = [0, 0, 1, 1]
    forest = ExtraTreesClassifier(random_state=0).fit(_X, _y)
    forest.predict(_X)
    return
