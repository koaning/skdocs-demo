...
def _(X_test, X_train):
    ...
    X_train_1 = mm.fit_transform(X_train)
    X_test_1 = mm.transform(X_test)
    return X_test_1, X_train_1, make_pipeline
...
@app.cell
def _(X_test_1, X_train_1, y_test, y_train):
    import time
    from sklearn.svm import LinearSVC
    results = {}
    lsvm = LinearSVC()
    _start = time.time()
    lsvm.fit(X_train_1, y_train)
    lsvm_time = time.time() - _start
    lsvm_score = 100 * lsvm.score(X_test_1, y_test)
    results['LSVM'] = {'time': lsvm_time, 'score': lsvm_score}
    print(f'Linear SVM score on raw features: {lsvm_score:.2f}%')
    return LinearSVC, results, time
...
def _(
    ...
    for _n_components in N_COMPONENTS:
        ...
        for _ in range(n_runs):
            pipeline = make_pipeline(PolynomialCountSketch(n_components=_n_components, degree=4), LinearSVC())
            _start = time.time()
            pipeline.fit(X_train_1, y_train)
            ps_lsvm_time = ps_lsvm_time + (time.time() - _start)
            ps_lsvm_score = ps_lsvm_score + 100 * pipeline.score(X_test_1, y_test)
        ps_lsvm_time = ps_lsvm_time / n_runs
        ps_lsvm_score = ps_lsvm_score / n_runs
        results[f'LSVM + PS({_n_components})'] = {'time': ps_lsvm_time, 'score': ps_lsvm_score}
        print(f'Linear SVM score on {_n_components} PolynomialCountSketch features: {ps_lsvm_score:.2f}%')
    return (N_COMPONENTS,)
...
@app.cell
def _(X_test_1, X_train_1, results, time, y_test, y_train):
    from sklearn.svm import SVC
    ksvm = SVC(C=500.0, kernel='poly', degree=4, coef0=0, gamma=1.0)
    _start = time.time()
    ksvm.fit(X_train_1, y_train)
    ksvm_time = time.time() - _start
    ksvm_score = 100 * ksvm.score(X_test_1, y_test)
...
@app.cell
def _(N_COMPONENTS, results):
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(7, 7))
    ax.scatter([results['LSVM']['time']], [results['LSVM']['score']], label='Linear SVM', c='green', marker='^')
    ax.scatter([results['LSVM + PS(250)']['time']], [results['LSVM + PS(250)']['score']], label='Linear SVM + PolynomialCountSketch', c='blue')
    for _n_components in N_COMPONENTS:
        ax.scatter([results[f'LSVM + PS({_n_components})']['time']], [results[f'LSVM + PS({_n_components})']['score']], c='blue')
        ax.annotate(f'n_comp.={_n_components}', (results[f'LSVM + PS({_n_components})']['time'], results[f'LSVM + PS({_n_components})']['score']), xytext=(-30, 10), textcoords='offset pixels')
    ax.scatter([results['KSVM']['time']], [results['KSVM']['score']], label='Kernel SVM', c='red', marker='x')
