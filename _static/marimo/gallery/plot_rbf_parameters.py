...
def _(X_1, np, y):
    ...
    return C_range, SVC, gamma_range, grid
...
@app.cell
def _(SVC, X_2d_1, y_2d):
    C_2d_range = [0.01, 1, 100.0]
    gamma_2d_range = [0.1, 1, 10.0]
    classifiers = []
    for _C in C_2d_range:
        for _gamma in gamma_2d_range:
            _clf = SVC(C=_C, gamma=_gamma)
            _clf.fit(X_2d_1, y_2d)
            classifiers.append((_C, _gamma, _clf))
    return C_2d_range, classifiers, gamma_2d_range
...
def _(
    ...
    for k, (_C, _gamma, _clf) in enumerate(classifiers):
        Z = _clf.decision_function(np.c_[xx.ravel(), yy.ravel()])
        Z = Z.reshape(xx.shape)
        plt.subplot(len(C_2d_range), len(gamma_2d_range), k + 1)
        plt.title('gamma=10^%d, C=10^%d' % (np.log10(_gamma), np.log10(_C)), size='medium')
        plt.pcolormesh(xx, yy, -Z, cmap=plt.cm.RdBu)
        plt.scatter(X_2d_1[:, 0], X_2d_1[:, 1], c=y_2d, cmap=plt.cm.RdBu_r, edgecolors='k')
        plt.xticks(())
        plt.yticks(())
        plt.axis('tight')
...
@app.cell
def _(C_range, MidpointNormalize, gamma_range, np, plt, scores):
    plt.figure(figsize=(8, 6))
