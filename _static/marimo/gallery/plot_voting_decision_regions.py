...
def _():
    ...
    xor = pd.DataFrame(np.random.RandomState(0).uniform(low=-1, high=1, size=(n_samples, 2)), columns=feature_names)
    noise = rng.normal(loc=0, scale=0.1, size=(n_samples, 2))
    target_xor = np.logical_xor(xor['Feature #0'] + noise[:, 0] > 0, xor['Feature #1'] + noise[:, 1] > 0)
    X = xor[feature_names]
    ...
    plt.show()
    return X, common_scatter_plot_params, np, pd, plt, y
...
def _(X, y):
    ...
    clf3.fit(X, y)
    eclf.fit(X, y)
    return clf1, clf2, clf3, eclf, weights
...
@app.cell
def _(X, clf1, clf2, clf3, common_scatter_plot_params, eclf, plt, y):
    from itertools import product
    from sklearn.inspection import DecisionBoundaryDisplay
    _fig, axarr = plt.subplots(2, 2, sharex='col', sharey='row', figsize=(10, 8))
    for idx, clf, title in zip(product([0, 1], [0, 1]), [clf1, clf2, clf3, eclf], ['Splines with\nconstant extrapolation', 'Splines with\nperiodic extrapolation', 'RBF Nystroem', 'Soft Voting']):
        disp = DecisionBoundaryDisplay.from_estimator(clf, X, response_method='predict_proba', plot_method='pcolormesh', cmap='RdBu', alpha=0.8, ax=axarr[idx[0], idx[1]])
        axarr[idx[0], idx[1]].scatter(X['Feature #0'], X['Feature #1'], c=y, **common_scatter_plot_params)
        axarr[idx[0], idx[1]].set_title(title)
        _fig.colorbar(disp.surface_, ax=axarr[idx[0], idx[1]], label='Probability estimate')
    plt.show()
    return
...
def _(eclf, pd):
    ...
    return predict_probas, test_sample


@app.cell
def _(np, predict_probas, weights):
    print(
