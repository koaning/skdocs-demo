...
def _():
    ...
    return X, X_test, X_train, scaled_X_train, scaler, y, y_test, y_train
...
def _(X, scaler, y):
    ...
    def fit_and_plot_model(X_plot, y, clf, ax):
        ...
        return disp.ax_
    _fig, (_ax1, _ax2) = plt.subplots(ncols=2, figsize=(12, 6))
    fit_and_plot_model(X_plot, y, clf, _ax1)
    _ax1.set_title('KNN without scaling')
    fit_and_plot_model(X_plot_scaled, y, clf, _ax2)
    _ax2.set_xlabel('scaled proline')
    _ax2.set_ylabel('scaled hue')
    _ = _ax2.set_title('KNN with scaling')
    return (plt,)
...
@app.cell
def _(X, X_train, plt, scaled_X_train):
    import pandas as pd
    ...
    _ = plt.tight_layout()
    return X_train_std_transformed, X_train_transformed, pca
...
@app.cell
def _(X_train_std_transformed, X_train_transformed, plt, y_train):
    _fig, (_ax1, _ax2) = plt.subplots(nrows=1, ncols=2, figsize=(10, 5))
    target_classes = range(0, 3)
    colors = ('blue', 'red', 'green')
    markers = ('^', 's', 'o')
    for target_class, color, marker in zip(target_classes, colors, markers):
        _ax1.scatter(x=X_train_transformed[y_train == target_class, 0], y=X_train_transformed[y_train == target_class, 1], color=color, label=f'class {target_class}', alpha=0.5, marker=marker)
        _ax2.scatter(x=X_train_std_transformed[y_train == target_class, 0], y=X_train_std_transformed[y_train == target_class, 1], color=color, label=f'class {target_class}', alpha=0.5, marker=marker)
    _ax1.set_title('Unscaled training dataset after PCA')
    _ax2.set_title('Standardized training dataset after PCA')
    for ax in (_ax1, _ax2):
...
@app.cell
def _(X_train, pca, scaler, y_train):
    import numpy as np
    ...
    return scaled_clf, unscaled_clf
...
@app.cell
def _(X_test, scaled_clf, unscaled_clf, y_test):
