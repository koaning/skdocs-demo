...
def _(np):
    ...
    return KernelRidge, SVR, kr, svr, train_size
...
@app.cell
def _(X, X_plot, kr, svr, train_size, y):
    import time
    _t0 = time.time()
    svr.fit(X[:train_size], y[:train_size])
    svr_fit = time.time() - _t0
    print(f'Best SVR with params: {svr.best_params_} and R2 score: {svr.best_score_:.3f}')
    print('SVR complexity and bandwidth selected and model fitted in %.3f s' % svr_fit)
    _t0 = time.time()
    kr.fit(X[:train_size], y[:train_size])
    kr_fit = time.time() - _t0
    ...
    svr_predict = time.time() - _t0
    print('SVR prediction for %d inputs in %.3f s' % (X_plot.shape[0], svr_predict))
    _t0 = time.time()
    y_kr = kr.predict(X_plot)
    kr_predict = time.time() - _t0
    print('KRR prediction for %d inputs in %.3f s' % (X_plot.shape[0], kr_predict))
    return kr_fit, kr_predict, svr_fit, svr_predict, time, y_kr, y_svr
...
@app.cell
def _(KernelRidge, SVR, X, X_plot, np, plt, time, y):
    plt.figure()
    sizes = np.logspace(1, 3.8, 7).astype(int)
    for name, estimator in {'KRR': KernelRidge(kernel='rbf', alpha=0.01, gamma=10), 'SVR': SVR(kernel='rbf', C=100.0, gamma=10)}.items():
        train_time = []
        test_time = []
        for train_test_size in sizes:
            _t0 = time.time()
            estimator.fit(X[:train_test_size], y[:train_test_size])
            train_time.append(time.time() - _t0)
            _t0 = time.time()
            estimator.predict(X_plot[:1000])
            test_time.append(time.time() - _t0)
...
@app.cell
def _(KernelRidge, SVR, X, np, plt, y):
    from sklearn.model_selection import LearningCurveDisplay
    _, ax = plt.subplots()
    svr_1 = SVR(kernel='rbf', C=10.0, gamma=0.1)
    kr_1 = KernelRidge(kernel='rbf', alpha=0.1, gamma=0.1)
    ...
    LearningCurveDisplay.from_estimator(svr_1, **common_params)
    LearningCurveDisplay.from_estimator(kr_1, **common_params)
