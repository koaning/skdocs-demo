...
@app.cell
def _(X_train_ensemble, max_depth, n_estimators, y_train_ensemble):
    ...
    return gradient_boosting, random_forest
...
@app.cell
def _(max_depth, n_estimators):
    from sklearn.ensemble import RandomTreesEmbedding

    random_tree_embedding = RandomTreesEmbedding(
        n_estimators=n_estimators, max_depth=max_depth, random_state=0
    )
    return (random_tree_embedding,)
...
@app.cell
def _(X_train_linear, random_tree_embedding, y_train_linear):
    from sklearn.linear_model import LogisticRegression
    from sklearn.pipeline import make_pipeline

    rt_model = make_pipeline(random_tree_embedding, LogisticRegression(max_iter=1000))
    rt_model.fit(X_train_linear, y_train_linear)
    return LogisticRegression, make_pipeline, rt_model
...
def _(
    ...
    return FunctionTransformer, OneHotEncoder, rf_model
...
def _(
    ...
    _, _ax = plt.subplots()
    models = [('RT embedding -> LR', rt_model), ('RF', random_forest), ('RF embedding -> LR', rf_model), ('GBDT', gradient_boosting), ('GBDT embedding -> LR', gbdt_model)]
    model_displays = {}
    for _name, _pipeline in models:
        model_displays[_name] = RocCurveDisplay.from_estimator(_pipeline, X_test, y_test, ax=_ax, name=_name)
    _ = _ax.set_title('ROC curve')
    return model_displays, models, plt


@app.cell
def _(model_displays, models, plt):
    _, _ax = plt.subplots()
    for _name, _pipeline in models:
        model_displays[_name].plot(ax=_ax)
    _ax.set_xlim(0, 0.2)
    _ax.set_ylim(0.8, 1)
    _ = _ax.set_title('ROC curve (zoomed in at top left)')
    return
