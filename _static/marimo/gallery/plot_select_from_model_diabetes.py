...
def _(X, diabetes, y):
    ...
    plt.show()
    return feature_names, importance, np, ridge
...
@app.cell
def _(X, feature_names, importance, np, ridge, y):
...
@app.cell
def _(X, feature_names, ridge, time, y):
...
def _(np):
    ...
    X_1, y_1 = (breast_cancer_data.data, breast_cancer_data.target)
    feature_names_1 = np.array(breast_cancer_data.feature_names)
    print(breast_cancer_data.DESCR)
    return X_1, feature_names_1, y_1
...
@app.cell
def _(SequentialFeatureSelector, X_1, feature_names_1, time, y_1):
    ...
    for tol in [-0.01, -0.001, -0.0001]:
        start = time()
        feature_selector = SequentialFeatureSelector(LogisticRegression(), n_features_to_select='auto', direction='backward', scoring='roc_auc', tol=tol, n_jobs=2)
        model = make_pipeline(StandardScaler(), feature_selector, LogisticRegression())
        model.fit(X_1, y_1)
        end = time()
        print(f'\ntol: {tol}')
        print(f'Features selected: {feature_names_1[model[1].get_support()]}')
        print(f'ROC AUC score: {roc_auc_score(y_1, model.predict_proba(X_1)[:, 1]):.3f}')
