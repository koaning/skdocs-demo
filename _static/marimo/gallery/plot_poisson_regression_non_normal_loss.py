...
def _(df, np, plt):
    ...
    _fig, (ax0, ax1, ax2) = plt.subplots(ncols=3, figsize=(16, 4))
...
def _(np):
    ...
    return ColumnTransformer, linear_model_preprocessor
...
@app.cell
def _(df, linear_model_preprocessor):
    ...
    return Pipeline, df_test, df_train, dummy
...
def _(df_test, dummy):
    ...
    def score_estimator(estimator, df_test):
        ...
        _y_pred = estimator.predict(df_test)
        print('MSE: %.3f' % mean_squared_error(df_test['Frequency'], _y_pred, sample_weight=df_test['Exposure']))
        print('MAE: %.3f' % mean_absolute_error(df_test['Frequency'], _y_pred, sample_weight=df_test['Exposure']))
        mask = _y_pred > 0
        if (~mask).any():
        ...
        print('mean Poisson deviance: %.3f' % mean_poisson_deviance(df_test['Frequency'][mask], _y_pred[mask], sample_weight=df_test['Exposure'][mask]))
    print('Constant mean frequency evaluation:')
    score_estimator(dummy, df_test)  # Ignore non-positive predictions, as they are invalid for  # the Poisson deviance.
...
@app.cell
def _(Pipeline, df_train, linear_model_preprocessor):
...
@app.cell
def _(df_test, ridge_glm, score_estimator):
...
@app.cell
def _(Pipeline, df_test, df_train, linear_model_preprocessor, score_estimator):
...
@app.cell
def _(ColumnTransformer, Pipeline, df_test, df_train, score_estimator):
...
@app.cell
def _(df_test, df_train, np, pd, plt, poisson_gbrt, poisson_glm, ridge_glm):
    _fig, axes = plt.subplots(nrows=2, ncols=4, figsize=(16, 6), sharey=True)
    _fig.subplots_adjust(bottom=0.2)
    n_bins = 20
    for row_idx, _label, df_1 in zip(range(2), ['train', 'test'], [df_train, df_test]):
        df_1['Frequency'].hist(bins=np.linspace(-1, 30, n_bins), ax=axes[row_idx, 0])
        axes[row_idx, 0].set_title('Data')
        axes[row_idx, 0].set_yscale('log')
        axes[row_idx, 0].set_xlabel('y (observed Frequency)')
        axes[row_idx, 0].set_ylim([10.0, 500000.0])
        axes[row_idx, 0].set_ylabel(_label + ' samples')
        for idx, _model in enumerate([ridge_glm, poisson_glm, poisson_gbrt]):
            _y_pred = _model.predict(df_1)
            pd.Series(_y_pred).hist(bins=np.linspace(-1, 4, n_bins), ax=axes[row_idx, idx + 1])
            axes[row_idx, idx + 1].set(title=_model[-1].__class__.__name__, yscale='log', xlabel='y_pred (predicted expected Frequency)')
    plt.tight_layout()
    return
...
@app.cell
def _(df_test, dummy, np, plt, poisson_gbrt, poisson_glm, ridge_glm):
    ...
    def _mean_frequency_by_risk_group(y_true, y_pred, sample_weight=None, n_bins=100):
        ...
        y_pred_bin: ndarray of shape (n_bins,)
            average y_pred for each bin
        """
        idx_sort = np.argsort(_y_pred)
        bin_centers = np.arange(0, 1, 1 / n_bins) + 0.5 / n_bins
        y_pred_bin = np.zeros(n_bins)
        y_true_bin = np.zeros(n_bins)
        for n, sl in enumerate(gen_even_slices(len(y_true), n_bins)):
            weights = sample_weight[idx_sort][sl]
            y_pred_bin[n] = np.average(_y_pred[idx_sort][sl], weights=weights)
    ...
    for axi, _model in zip(_ax.ravel(), [ridge_glm, poisson_glm, poisson_gbrt, dummy]):
        _y_pred = _model.predict(df_test)
        y_true = df_test['Frequency'].values
        exposure = df_test['Exposure'].values
        q, y_true_seg, y_pred_seg = _mean_frequency_by_risk_group(y_true, _y_pred, sample_weight=exposure, n_bins=10)
        print(f'Predicted number of claims by {_model[-1]}: {np.sum(_y_pred * exposure):.1f}')
        axi.plot(q, y_pred_seg, marker='x', linestyle='--', label='predictions')
        axi.plot(q, y_true_seg, marker='o', linestyle='--', label='observations')
        axi.set_xlim(0, 1.0)
        axi.set_ylim(0, 0.5)
        axi.set(title=_model[-1], xlabel='Fraction of samples sorted by y_pred', ylabel='Mean Frequency (y_pred)')
        axi.legend()
    plt.tight_layout()  # Name of the model after the estimator used in the last step of the  # pipeline.
    return
...
def _(df_test, dummy, np, plt, poisson_gbrt, poisson_glm, ridge_glm):
    ...
    def lorenz_curve(y_true, y_pred, exposure):
        y_true, _y_pred = (np.asarray(y_true), np.asarray(_y_pred))
        exposure = np.asarray(exposure)
        ranking = np.argsort(_y_pred)
    ...
    for _model in [dummy, ridge_glm, poisson_glm, poisson_gbrt]:
        _y_pred = _model.predict(df_test)
        cum_exposure, cum_claims = lorenz_curve(df_test['Frequency'], _y_pred, df_test['Exposure'])
        gini = 1 - 2 * auc(cum_exposure, cum_claims)
        _label = '{} (Gini: {:.2f})'.format(_model[-1], gini)
        _ax.plot(cum_exposure, cum_claims, linestyle='-', label=_label)
    cum_exposure, cum_claims = lorenz_curve(df_test['Frequency'], df_test['Frequency'], df_test['Exposure'])
    gini = 1 - 2 * auc(cum_exposure, cum_claims)
    _label = 'Oracle (Gini: {:.2f})'.format(gini)
    _ax.plot(cum_exposure, cum_claims, linestyle='-.', color='gray', label=_label)
    _ax.plot([0, 1], [0, 1], linestyle='--', color='black', label='Random baseline')
    _ax.set(title='Lorenz curves by model', xlabel='Cumulative proportion of exposure (from safest to riskiest)', ylabel='Cumulative proportion of claims')
