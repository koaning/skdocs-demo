...
def _():
    ...
    _model = make_pipeline(StandardScaler(with_std=False), LogisticRegression(C=2.0))
    _model
...
def _(StandardScaler):
    ...
    X_train, X_val, y_train, y_val = train_test_split(_X, _y, test_size=0.2, random_state=0)
    _clf = HistGradientBoostingClassifier()
    _clf.set_fit_request(X_val=True, y_val=True)
    _model = Pipeline([('sc', StandardScaler()), ('clf', _clf)], transform_input=['X_val'])
    _model.fit(_X, _y, X_val=X_val, y_val=y_val)
...
@app.cell
def _(LogisticRegression, make_classification):
    from sklearn.metrics import RocCurveDisplay
    from sklearn.model_selection import cross_validate
    _X, _y = make_classification(n_samples=150, random_state=0)
    _clf = LogisticRegression(random_state=0)
    cv_results = cross_validate(_clf, _X, _y, cv=5, return_estimator=True, return_indices=True)
    _ = RocCurveDisplay.from_cv_results(cv_results, _X, _y)
    return
