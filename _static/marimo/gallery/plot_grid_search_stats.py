...
def _():
    ...
    def corrected_std(differences, n_train, n_test):
        ...
        """
        kr = len(differences)
        corrected_var = np.var(differences, ddof=1) * (1 / kr + n_test / n_train)  # kr = k times r, r times repeated k-fold crossvalidation,
        corrected_std = np.sqrt(corrected_var)  # kr equals the number of times the model was evaluated
    ...
    def compute_corrected_ttest(differences, df, n_train, n_test):
        ...
        _t_stat = mean / std
        _p_val = t.sf(np.abs(_t_stat), df)
        return (_t_stat, _p_val)  # right-tailed t-test
    return compute_corrected_ttest, corrected_std, np, t


@app.cell
def _(X, compute_corrected_ttest, cv, model_scores, y):
    model_1_scores = model_scores.iloc[0].values  # scores of the best model
    model_2_scores = model_scores.iloc[1].values  # scores of the second-best model
    differences = model_1_scores - model_2_scores
    n = differences.shape[0]
    df = n - 1
    n_train = len(next(iter(cv.split(X, y)))[0])  # number of test sets
    n_test = len(next(iter(cv.split(X, y)))[1])
    _t_stat, _p_val = compute_corrected_ttest(differences, df, n_train, n_test)
    print(f'Corrected t-value: {_t_stat:.3f}\nCorrected p-value: {_p_val:.3f}')
    return df, differences, n, n_test, n_train
...
@app.cell
def _(corrected_std, df, differences, n_test, n_train, np, t):
...
@app.cell
def _(model_scores, t_post):
    _better_prob = 1 - t_post.cdf(0)
    print(f'Probability of {model_scores.index[0]} being more accurate than {model_scores.index[1]}: {_better_prob:.3f}')
    print(f'Probability of {model_scores.index[1]} being more accurate than {model_scores.index[0]}: {1 - _better_prob:.3f}')
    return
...
def _(model_scores, t_post):
    ...
    _rope_prob = t_post.cdf(rope_interval[1]) - t_post.cdf(rope_interval[0])
    print(f'Probability of {model_scores.index[0]} and {model_scores.index[1]} being practically equivalent: {_rope_prob:.3f}')
    return (rope_interval,)
...
@app.cell
def _(np, plt, rope_interval, t_post, x):
...
@app.cell
def _(compute_corrected_ttest, df, model_scores, n_test, n_train, pd):
    from itertools import combinations
    from math import factorial
    n_comparisons = factorial(len(model_scores)) / (factorial(2) * factorial(len(model_scores) - 2))
    pairwise_t_test = []
    for _model_i, _model_k in combinations(range(len(model_scores)), 2):
        _model_i_scores = model_scores.iloc[_model_i].values
        _model_k_scores = model_scores.iloc[_model_k].values
        differences_1 = _model_i_scores - _model_k_scores
        _t_stat, _p_val = compute_corrected_ttest(differences_1, df, n_train, n_test)
        _p_val = _p_val * n_comparisons
        _p_val = 1 if _p_val > 1 else _p_val
        pairwise_t_test.append([model_scores.index[_model_i], model_scores.index[_model_k], _t_stat, _p_val])
    pairwise_comp_df = pd.DataFrame(pairwise_t_test, columns=['model_1', 'model_2', 't_stat', 'p_val']).round(3)
    pairwise_comp_df
    return combinations, pairwise_comp_df
...
def _(
    ...
    for _model_i, _model_k in combinations(range(len(model_scores)), 2):
        _model_i_scores = model_scores.iloc[_model_i].values
        _model_k_scores = model_scores.iloc[_model_k].values
        differences_2 = _model_i_scores - _model_k_scores
        t_post_1 = t(df, loc=np.mean(differences_2), scale=corrected_std(differences_2, n_train, n_test))
        worse_prob = t_post_1.cdf(rope_interval[0])
        _better_prob = 1 - t_post_1.cdf(rope_interval[1])
        _rope_prob = t_post_1.cdf(rope_interval[1]) - t_post_1.cdf(rope_interval[0])
        pairwise_bayesian.append([worse_prob, _better_prob, _rope_prob])
    pairwise_bayesian_df = pd.DataFrame(pairwise_bayesian, columns=['worse_prob', 'better_prob', 'rope_prob']).round(3)
    pairwise_comp_df_1 = pairwise_comp_df.join(pairwise_bayesian_df)
    pairwise_comp_df_1
    return
