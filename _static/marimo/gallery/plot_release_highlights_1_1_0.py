...
def _():
    ...
    _X = X_1d.reshape(-1, 1)
    _y = X_1d * np.cos(X_1d) + _rng.normal(scale=X_1d / 3)
    quantiles = [0.95, 0.5, 0.05]
    parameters = dict(loss='quantile', max_bins=32, max_iter=50)
    hist_quantiles = {f'quantile={quantile:.2f}': HistGradientBoostingRegressor(**parameters, quantile=quantile).fit(_X, _y) for quantile in quantiles}
    _fig, _ax = plt.subplots()
    _ax.plot(X_1d, _y, 'o', alpha=0.5, markersize=1)
    for quantile, hist in hist_quantiles.items():
        _ax.plot(X_1d, hist.predict(_X), label=quantile)
    _ = _ax.legend(loc='lower left')
    return np, plt
...
def _():
    ...
    _X, _y = fetch_openml('titanic', version=1, as_frame=True, return_X_y=True, parser='pandas')
    numeric_features = ['age', 'fare']
    numeric_transformer = make_pipeline(SimpleImputer(strategy='median'), StandardScaler())
    categorical_features = ['embarked', 'pclass']
    preprocessor = ColumnTransformer([('num', numeric_transformer, numeric_features), ('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False), categorical_features)], verbose_feature_names_out=False)
    log_reg = make_pipeline(preprocessor, SelectKBest(k=7), LogisticRegression())
    log_reg.fit(_X, _y)
    return OneHotEncoder, log_reg
...
def _(OneHotEncoder, np):
    _X = np.array([['dog'] * 5 + ['cat'] * 20 + ['rabbit'] * 10 + ['snake'] * 3], dtype=object).T
    enc = OneHotEncoder(min_frequency=6, sparse_output=False).fit(_X)
...
def _(np):
    ...
    true_W = _rng.uniform(size=(n_samples, n_components))
    true_H = _rng.uniform(size=(n_components, n_features))
    _X = true_W @ true_H
    ...
    H = nmf.components_
    X_reconstructed = W @ H
    print('relative reconstruction error: ', f'{np.sum((_X - X_reconstructed) ** 2) / np.sum(_X ** 2):.5f}')
    return
...
def _(plt):
    ...
    _X, _ = make_blobs(n_samples=1000, centers=2, random_state=0)
    km = KMeans(n_clusters=5, random_state=0, n_init='auto').fit(_X)
    bisect_km = BisectingKMeans(n_clusters=5, random_state=0).fit(_X)
    _fig, _ax = plt.subplots(1, 2, figsize=(10, 5))
    _ax[0].scatter(_X[:, 0], _X[:, 1], s=10, c=km.labels_)
    _ax[0].scatter(km.cluster_centers_[:, 0], km.cluster_centers_[:, 1], s=20, c='r')
    _ax[0].set_title('KMeans')
    _ax[1].scatter(_X[:, 0], _X[:, 1], s=10, c=bisect_km.labels_)
    _ax[1].scatter(bisect_km.cluster_centers_[:, 0], bisect_km.cluster_centers_[:, 1], s=20, c='r')
    _ = _ax[1].set_title('BisectingKMeans')
    return
