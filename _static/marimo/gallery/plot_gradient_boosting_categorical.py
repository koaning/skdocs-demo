...
def _(
    ...
    _dropped_result = cross_validate(hist_dropped, X, y, **common_params)
    _one_hot_result = cross_validate(hist_one_hot, X, y, **common_params)
    _ordinal_result = cross_validate(hist_ordinal, X, y, **common_params)
    _target_result = cross_validate(hist_target, X, y, **common_params)
    _native_result = cross_validate(hist_native, X, y, **common_params)
    results = [('Dropped', _dropped_result), ('One Hot', _one_hot_result), ('Ordinal', _ordinal_result), ('Target', _target_result), ('Native', _native_result)]
    return common_params, cross_validate, results
...
def _(np, results):
    ...
    return (plot_performance_tradeoff,)
...
def _(
    ...
    for pipe in (hist_dropped, hist_one_hot, hist_ordinal, hist_target, hist_native):
        if pipe is hist_native:
            pipe.set_params(max_depth=3, max_iter=15)  # The native model does not use a pipeline so, we can set the parameters
        else:  # directly.
    ...
    _target_result = cross_validate(hist_target, X, y, **common_params)
    _native_result = cross_validate(hist_native, X, y, **common_params)
    results_underfit = [('Dropped', _dropped_result), ('One Hot', _one_hot_result), ('Ordinal', _ordinal_result), ('Target', _target_result), ('Native', _native_result)]
    return (results_underfit,)


@app.cell
def _(plot_performance_tradeoff, results_underfit):
