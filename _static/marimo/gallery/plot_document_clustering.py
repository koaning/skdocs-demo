...
def _():
    ...
    return category_sizes, dataset, labels, np, true_k
...
def _(labels, np):
    ...
    return evaluations, evaluations_std, fit_and_evaluate, time
...
def _(dataset, time):
    ...
    print(f'vectorization done in {time() - _t0:.3f} s')
...
@app.cell
def _(X_tfidf, category_sizes, np, true_k):
...
@app.cell
def _(KMeans, X_tfidf, fit_and_evaluate, true_k):
    kmeans_1 = KMeans(n_clusters=true_k, max_iter=100, n_init=5)
    fit_and_evaluate(kmeans_1, X_tfidf, name='KMeans\non tf-idf vectors')
    return
...
def _(X_tfidf, time):
    ...
    print(f'LSA done in {time() - _t0:.3f} s')
    print(f'Explained variance of the SVD step: {explained_variance * 100:.1f}%')
    return Normalizer, TruncatedSVD, X_lsa, lsa, make_pipeline
...
@app.cell
def _(KMeans, X_lsa, fit_and_evaluate, true_k):
    kmeans_2 = KMeans(n_clusters=true_k, max_iter=100, n_init=1)
    fit_and_evaluate(kmeans_2, X_lsa, name='KMeans\nwith LSA on tf-idf vectors')
    return (kmeans_2,)
...
@app.cell
def _(X_lsa, fit_and_evaluate, true_k):
...
@app.cell
def _(kmeans_2, lsa, true_k, vectorizer):
    original_space_centroids = lsa[0].inverse_transform(kmeans_2.cluster_centers_)
...
@app.cell
def _(Normalizer, TruncatedSVD, dataset, make_pipeline, time):
...
@app.cell
def _(X_hashed_lsa, fit_and_evaluate, kmeans_2):
    fit_and_evaluate(kmeans_2, X_hashed_lsa, name='KMeans\nwith LSA on hashed vectors')
    return


@app.cell
def _(X_hashed_lsa, fit_and_evaluate, minibatch_kmeans):
    fit_and_evaluate(
        minibatch_kmeans,
        X_hashed_lsa,
        name="MiniBatchKMeans\nwith LSA on hashed vectors",
...
@app.cell
def _(evaluations, evaluations_std):
