...
@app.cell
def _(PartialDependenceDisplay, X, plt, tree):
    _fig, _ax = plt.subplots(figsize=(12, 6))
    _ax.set_title('Decision Tree')
    tree_disp = PartialDependenceDisplay.from_estimator(tree, X, ['age', 'bmi'], ax=_ax)
    return (tree_disp,)
...
@app.cell
def _(PartialDependenceDisplay, X, mlp, plt):
    _fig, _ax = plt.subplots(figsize=(12, 6))
    _ax.set_title('Multi-layer Perceptron')
    mlp_disp = PartialDependenceDisplay.from_estimator(mlp, X, ['age', 'bmi'], ax=_ax, line_kw={'color': 'red'})
    return (mlp_disp,)
...
@app.cell
def _(mlp_disp, plt, tree_disp):
    _fig, (_ax1, _ax2) = plt.subplots(2, 1, figsize=(10, 10))
    tree_disp.plot(ax=_ax1)
    _ax1.set_title('Decision Tree')
    mlp_disp.plot(ax=_ax2, line_kw={'color': 'red'})
    _ax2.set_title('Multi-layer Perceptron')
    return
...
def _(mlp_disp, plt, tree_disp):
    ...
    tree_disp.plot(ax=[_ax1, _ax2], line_kw={'label': 'Decision Tree'})
    mlp_disp.plot(ax=[_ax1, _ax2], line_kw={'label': 'Multi-layer Perceptron', 'color': 'red'})
    _ax1.legend()
    _ax2.legend()
    return
...
@app.cell
def _(PartialDependenceDisplay, X, mlp, tree):
    tree_disp_1 = PartialDependenceDisplay.from_estimator(tree, X, ['age'])
    mlp_disp_1 = PartialDependenceDisplay.from_estimator(mlp, X, ['age'], ax=tree_disp_1.axes_, line_kw={'color': 'red'})
    return
