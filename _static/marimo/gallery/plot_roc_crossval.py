...
def _():
    ...
    return X, n_features, n_samples, np, target_names, y
...
@app.cell
def _(X, n_features, n_samples, np):
    random_state = np.random.RandomState(0)
    X_1 = np.concatenate([X, random_state.randn(n_samples, 200 * n_features)], axis=1)
    return X_1, random_state
...
@app.cell
def _(X_1, np, random_state, target_names, y):
    ...
    cv_results = cross_validate(classifier, X_1, y, cv=cv, return_estimator=True, return_indices=True)
    ...
    viz = RocCurveDisplay.from_cv_results(cv_results, X_1, y, ax=ax, name=names, curve_kwargs=curve_kwargs_list, plot_chance_level=True)
    ...
    ax.plot(mean_fpr, mean_tpr, color='b', label='Mean ROC (AUC = %0.2f $\\pm$ %0.2f)' % (mean_auc, std_auc), lw=2, alpha=0.8)
