...
def _(plt, raccoon_face):
    ...
    fig_1, ax_1 = plt.subplots(ncols=2, figsize=(12, 4))
    ax_1[0].imshow(compressed_raccoon_uniform, cmap=plt.cm.gray)
    ax_1[0].axis('off')
    ax_1[0].set_title('Rendering of the image')
    ax_1[1].hist(compressed_raccoon_uniform.ravel(), bins=256)
    ax_1[1].set_xlabel('Pixel value')
    ax_1[1].set_ylabel('Count of pixels')
    ax_1[1].set_title('Sub-sampled distribution of the pixel values')
    _ = fig_1.suptitle('Raccoon face compressed using 3 bits and a uniform strategy')
    return KBinsDiscretizer, encoder, n_bins
...
@app.cell
def _(bin_center, plt, raccoon_face):
    _, ax_2 = plt.subplots()
    ax_2.hist(raccoon_face.ravel(), bins=256)
    color = 'tab:orange'
    for center in bin_center:
        ax_2.axvline(center, color=color)
        ax_2.text(center - 10, ax_2.get_ybound()[1] + 100, f'{center:.1f}', color=color)
    return
...
@app.cell
def _(KBinsDiscretizer, n_bins, plt, raccoon_face):
    encoder_1 = KBinsDiscretizer(n_bins=n_bins, encode='ordinal', strategy='kmeans', random_state=0)
    compressed_raccoon_kmeans = encoder_1.fit_transform(raccoon_face.reshape(-1, 1)).reshape(raccoon_face.shape)
    fig_2, ax_3 = plt.subplots(ncols=2, figsize=(12, 4))
    ax_3[0].imshow(compressed_raccoon_kmeans, cmap=plt.cm.gray)
    ax_3[0].axis('off')
    ax_3[0].set_title('Rendering of the image')
    ax_3[1].hist(compressed_raccoon_kmeans.ravel(), bins=256)
    ax_3[1].set_xlabel('Pixel value')
    ax_3[1].set_ylabel('Number of pixels')
    ax_3[1].set_title('Distribution of the pixel values')
    _ = fig_2.suptitle('Raccoon face compressed using 3 bits and a K-means strategy')
    return compressed_raccoon_kmeans, encoder_1


@app.cell
def _(encoder_1):
    bin_edges_1 = encoder_1.bin_edges_[0]
    bin_center_1 = bin_edges_1[:-1] + (bin_edges_1[1:] - bin_edges_1[:-1]) / 2
    bin_center_1
    return (bin_center_1,)


@app.cell
def _(bin_center_1, plt, raccoon_face):
    _, ax_4 = plt.subplots()
    ax_4.hist(raccoon_face.ravel(), bins=256)
    color_1 = 'tab:orange'
    for center_1 in bin_center_1:
        ax_4.axvline(center_1, color=color_1)
        ax_4.text(center_1 - 10, ax_4.get_ybound()[1] + 100, f'{center_1:.1f}', color=color_1)
    return
...
@app.cell
def _(compressed_raccoon_kmeans, raccoon_face):
...
@app.cell
def _(compressed_raccoon_kmeans):
