...
def _():
    ...
    X -= X.mean(axis=0)
    X /= X.std(axis=0)
    return X, cov, linalg, n_samples, np, prec
...
def _(X, linalg, n_samples, np):
    ...
    return cov_, emp_cov, lw_cov_, lw_prec_, model, prec_
...
@app.cell
def _(cov, cov_, emp_cov, linalg, lw_cov_, lw_prec_, np, prec, prec_):
